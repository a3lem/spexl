...
The system SHALL support `spexl onboard` to print a short primer intended for manual paste into a project's AGENTS.md or CLAUDE.md. The primer names the five action skills, the librarian skill, and the core rules, and points at the librarian for deeper methodology knowledge. The command writes the primer content to stdout and a paste-instruction header to stderr, so that redirection (`spexl onboard >> AGENTS.md`) captures only the primer.

#### Scenario: Default output
- **WHEN** the user runs `spexl onboard`
- **THEN** the system prints the primer to stdout containing: a one-line statement that specs are the source of truth, the five action skills with their trigger phrases, the core rules, the directory layout, and a pointer to the `learn-about-sdd-with-spexl` skill
