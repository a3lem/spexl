...
├── cli/
│   ├── __init__.py
│   ├── plumbing.py      # Existing spectl commands (new, changes, validate, etc.)
...
The system SHALL use argparse with subparsers to route to command handlers. Each subcommand group (plumbing, generation, steering) registers its own subparsers.
