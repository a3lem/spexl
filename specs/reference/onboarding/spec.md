...
Onboarding emits a short primer describing the spexl workflow, intended for manual paste into a project's `AGENTS.md` or `CLAUDE.md`. The primer gives an agent enough context to recognize spexl-relevant work and route it to the correct skill; deeper methodology knowledge is served by the `spexl-foundations` skill that action skills invoke.
