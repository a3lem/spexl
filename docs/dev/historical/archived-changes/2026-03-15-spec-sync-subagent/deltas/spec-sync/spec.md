...
The spec-sync agent SHALL merge each delta spec in a change's `deltas/` directory into the corresponding reference spec. The merge operates at three levels of hierarchy: OPERATOR (ADDED/MODIFIED/REMOVED/RENAMED) > REQUIREMENT > SCENARIO.
...
IF `specs/config.toml` contains a `[workspaces]` table, the spec-sync agent SHALL match delta specs to workspace-scoped reference directories.

#### Scenario: Workspace-scoped sync
  Given a monorepo with workspace `backend`
  And a delta at `specs/backend/changes/add-oauth/deltas/user-auth/spec.md`
  When the spec-sync agent processes the delta
  Then it merges into `specs/backend/reference/user-auth/spec.md`
